...
def AddIndicators(df):
    ...
    indicator_bb = KeltnerChannel(high=df["High"],low=df["Low"],close=df["Close"], window=6, window_atr=10)
    df['kc_mb'] = indicator_bb.keltner_channel_mband()
    df['kc_hb'] = indicator_bb.keltner_channel_hband()
    df['kc_lb'] = indicator_bb.keltner_channel_lband()
    df['kc_pb'] = indicator_bb.keltner_channel_pband()
    df['kc_wb'] = indicator_bb.keltner_channel_wband()
    df['kc_hb_i'] = indicator_bb.keltner_channel_hband_indicator()
    df['kc_lb_i'] = indicator_bb.keltner_channel_lband_indicator()
...
if __name__ == "__main__":
    ...
    df = pd.read_csv('./Binance_BTCUSDT_8h_Modified.csv')
    df = df.sort_values('Date')
    df = AddIndicators(df)
    ...
    # Add Bollinger Bands
    Plot_OHCL(test_df, ax1_indicators=['kc_lb','kc_hb','kc_mb'])
